    ...
    state.scanSocket.onmessage = function(event) {
      ...
      if (msg.processing) {
        ...
        if (state.currentScanUrl) {
          ...
          setTimeout(() => {
            checkScanHistoryForResults(CONFIG.username || 'guest', state.currentScanUrl);
          }, 5000);
        }
  ...
  try {
    ...
    setTimeout(() => {
      ...
      if (state.currentScanUrl === url) {
        checkScanHistoryForResults(username, url);
      }
...
function pingScanSocket() {
  if (state.scanSocket && state.scanSocket.readyState === WebSocket.OPEN) {
    ...
    try {
      state.scanSocket.send(JSON.stringify({ type: 'ping' }));
    } catch (e) {
...
function processScanResult(responseText, url) {
  ...
  try {
    ...
    // Create a simplified HTML string with no extra spacing
    const resultHtml = `<strong>URL:</strong> ${link}<br><strong>Analysis:</strong> ${analysis}`;
    
    // Use addScanResultMessage for proper styling
    addScanResultMessage(resultHtml);
    
    // Add to history if function exists
    if (typeof addToScanHistory === 'function') {
      addToScanHistory(link, analysis);
    }
...
function checkScanHistoryForResults(username, url) {
  ...
  // Always add API key when available (IMPORTANT FIX)
  if (CONFIG.apiKey) {
...
// Initialize on DOM load - make sure this is kept
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Ensure username is set
  if (!CONFIG.username) {
    console.log('Setting CONFIG.username on load to "guest"');
    CONFIG.username = 'guest';
  }
  
  // Make sure CONFIG.apiKey is set for API requests
  if (!CONFIG.apiKey && typeof API_KEY !== 'undefined') {
    CONFIG.apiKey = API_KEY; // Use global API_KEY if available
    console.log('Using global API_KEY for requests');
  }
  ...
  if (CONFIG.pingInterval !== false) {
    setInterval(pingScanSocket, CONFIG.pingInterval || 30000);
  }
});
...
// Add system message with system label
