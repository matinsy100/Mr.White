...
# === Constants ===
VERSION = "1.0.0"
MAX_SCAN_HISTORY = 1  # MODIFIED: Changed from 5 to 1 to avoid token glitches
MAX_MEMORY_TURNS = 10
...
async def process_chat(user: str, msg: str) -> str:
    ...
    # NEW CODE: Check if there's a recent scan to include in context
...
async def scan_endpoint(data: URLScanRequest = Body(...)):
    ...
    try:
        ...
        try:
                    ...
                    messages=[
                        {
                            "role": "system", 
                            "content": (
                                "You are Mr. White, a content analysis specialist and cybersecurity assistant.\n"
                                "You MUST reply in the following format only:\n\n"
                                "Status: <threat assessment summary>\n"
                                "Threat Level: <Safe|Low|Medium|High|Critical>\n"
                                "Link: <URL>\n\n"
                                "1. Main purpose of the website\n"
                                "2. Key topics or information presented\n"
                                "3. Important features or functionality\n\n"
                                "If any technical anomalies or security concerns are found (such as invalid query parameters, malformed URLs, etc.), "
                                "you must mention them briefly and recommend investigation steps.\n\n"
                                "Be concise, professional, and focused on both content and security insights.\n\n"
                                "Threat Level Definitions:\n"
                                "- Safe: No security concerns detected\n"
                                "- Low: Minor concerns, generally safe to proceed\n" 
                                "- Medium: Some suspicious elements that warrant caution\n"
                                "- High: Potentially malicious, proceed with extreme caution\n"
                                "- Critical: Dangerous site, do not proceed"
                            )
                        },
                        {"role": "user", "content": formatted_message}
                    ]
                )),
                timeout=25  # Slightly increased timeout for more detailed analysis
            )
            ...
            # MODIFIED: Replace scan history instead of appending
            # Create a new scan history with just this newest scan
            scans = [{"page": formatted_message, "result": reply}]
    ...
    try:
        ...
        while True:
            try:
                ...
                try:
                    await ws.send.json({"status": "Scan Completed"})
                except Exception:
            ...
            try:
                            ...
                            messages=[
                                {
                                    "role": "system", 
                                    "content": (
                                        "You are Mr. White, an advanced Website Cybersecurity Specialist. Your role is to thoroughly analyze any provided website for signs of malicious intent, suspicious behavior, or potential security threats. Deliver concise, actionable insights to protect users and ensure web safety."
                                    )
                                },
                                {"role": "user", "content": formatted_message}
                            ]
                ...
                # MODIFIED: Save only the latest scan
                try:
                    ...
                    # Create a new scan history with just this one scan
                    scans = [{"page": shortened_content, "result": reply}]
                    save_scan_history(user, scans)
